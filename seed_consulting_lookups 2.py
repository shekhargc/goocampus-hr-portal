"""
seed_consulting_lookups.py — auto-populate lookup_options for pathway='consulting'.

After Phase 2 imports landed 5,500+ records of historical Standard Consulting data into
the ops_* tables, the user expected Settings -> Standard Consulting tab to
...
becomes a selectable option, scoped to pathway='consulting'.
...
sees its values, Consulting sees Consulting values — strict per-pathway
isolation enforced by the pathway column added in Phase 1.

Idempotent via _import_markers marker key 'cs_lookups_seed'. Bump
IMPORT_VERSION to force a re-seed (e.g. after adding a new mapping).
Runs on app boot AFTER the Consulting S-3 imports so the source data
...
IMPORT_VERSION = 'cs_lookups_v3_force_reseed_after_data_imported'
...
# Source rows are always filtered to pathway='consulting'.
LOOKUP_SOURCES = [
    # ── Test Bookings ──
...
def run_seed_consulting_lookups_once(get_db_fn):
    """Seed lookup_options for pathway='consulting' from imported ops_* data.
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                pass
        marker = conn.execute(
            "SELECT value FROM _import_markers WHERE key = 'cs_lookups_seed'"
        ).fetchone()
        if marker and marker['value'] == IMPORT_VERSION:
            logging.info(f"Consulting lookups seed: already at {IMPORT_VERSION}, skipping")
            return {'per_category': {}, 'skipped_categories': []}

        logging.info(f"Consulting lookups seed: starting {IMPORT_VERSION}...")
        ...
        for category, table, column in LOOKUP_SOURCES:
            ...
            # Pull distinct values from rows scoped to pathway='consulting'.
            try:
                rows = conn.execute(
                    f"SELECT DISTINCT TRIM({column}) AS v FROM {table} "
                    f"WHERE pathway = 'consulting' "
            ...
            inserted = 0
            updated = 0
            # Highest existing sort_order for this (category, consulting) so we
            ...
            for value in distinct_values:
                try:
                    existing = conn.execute(
                        ...
                        "WHERE category = ? AND value = ? AND COALESCE(pathway, 'plab') = 'consulting'",
                        (category, value),
                    ).fetchone()
                    if existing:
                    ...
                    conn.execute(
                        ...
                        "VALUES (?, ?, ?, ?, ?, ?)",
                        (category, value, value, max_order, 1, 'consulting'),
                    )
                    inserted += 1
                except Exception as e:
        ...
        try:
            conn.execute(
                "INSERT INTO _import_markers (key, value) VALUES ('cs_lookups_seed', ?) "
            ...
            try:
                conn.execute("DELETE FROM _import_markers WHERE key = 'cs_lookups_seed'")
                conn.execute(
                    "INSERT INTO _import_markers (key, value) VALUES ('cs_lookups_seed', ?)",
                    (IMPORT_VERSION,),
                )
        ...
        logging.info(
            f"Consulting lookups seed {IMPORT_VERSION} complete — "
