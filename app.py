...
try:
    import xlrd
    logging.info(f"xlrd {xlrd.__VERSION__} loaded successfully")
except ImportError:
    logging.warning("xlrd is NOT installed — .xls file uploads will require conversion to .xlsx")
...
def partner_dashboard_view():
    ...
    # Get visible sections for this partner
...
@app.route('/partner/medical-predictor')
...
def pending_approvals():
    ...
    def _build_where(status_filter):
        where = [status_filter]
        params = []
        # Role-based access: admin sees all, manager sees self + reports, employee sees own
        if user['is_admin']:
            pass  # no filter
        else:
            # Check if user has any reports
            has_reports = conn.execute('SELECT 1 FROM employees WHERE reporting_to = ? LIMIT 1', (user['id'],)).fetchone()
            if has_reports:
                # Manager: own + direct reports
                where.append('(e.id = ? OR e.reporting_to = ?)')
    ...
    approved_count = sum(1 for a in approved if a.get('status') == 'approved')
    rejected_count = sum(1 for a in approved if a.get('status') == 'rejected')

    conn.close()

    return render_template('pending_approvals.html', user=user,
                         pending_leaves=pending, approved_leaves=approved,
...
def ensure_ops_tables():
    ...
    try:
        ...
        SEED_DATA = {
                ...
                'training_program': ['IELTS Training', 'OET Training', 'PLAB 1 Training', 'PLAB 2 Training', 'MRCP 1', 'PLAB 2 Mock'],
                'ielts_vendor': ['Verbis', 'Britford Training Academy', 'Other'],
                'oet_vendor': ['Verbis', 'Britford Training Academy', 'Other'],
                'plab1_partner': ['ABMA', 'Arora PLAB 1', 'Other'],
                'plab2_vendor': ['Aspire Academy', 'ABMA & Aspire', 'Arora PLAB 2', 'Other'],
                'other_training_vendor': ['Other'],
...
@admin_required
def ops_uk_pathway():
    """UK Pathway — redirects to the pathway dashboard with sidebar."""
...
CATEGORY_LABELS = {
    ...
    'ielts_vendor': 'IELTS Vendors',
    'oet_vendor': 'OET Vendors',
    'plab1_partner': 'PLAB 1 Partners',
    'plab2_vendor': 'PLAB 2 Vendors',
    'other_training_vendor': 'Other Vendors',
...
def ops_english_logins_list():
    ...
    return render_template('ops_english_logins_list.html', records=records, search=search,
                           active_ops_page='english-logins')
...
def ops_test_bookings_add():
    ...
    return render_template('ops_test_bookings_form.html', record=None, clients=clients,
                           ...
                           pre_reg=pre_reg, active_ops_page='test-bookings')
...
def ops_test_bookings_edit(record_id):
    ...
    return render_template('ops_test_bookings_form.html', record=record, clients=clients,
                           ...
                           exam_booked_by=get_lookup_options('exam_booked_by'), exam_countries=get_lookup_options('exam_country'),
                           revaluation_options=get_lookup_options('revaluation_option'), reval_results=get_lookup_options('reval_result'),
                           pre_reg='', active_ops_page='test-bookings')
...
def ops_epic_add():
    ...
    return render_template('ops_epic_form.html', record=None, clients=clients,
                           ...
                           pre_reg=pre_reg, active_ops_page='epic')
...
def ops_gmc_add():
    ...
    return render_template('ops_gmc_form.html', record=None, clients=clients,
                           ...
                           gmc_english_exams=get_lookup_options('gmc_english_exam'),
                           gmc_license_statuses=get_lookup_options('gmc_license_status'),
                           pre_reg=pre_reg, active_ops_page='gmc')
...
def ops_epic_import():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash(f'Import error: {e}', 'error')
            return redirect(request.url)
    return render_template('ops_epic_import.html', active_ops_page='epic')
...
def ops_gmc_import():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash(f'Import error: {e}', 'error')
            return redirect(request.url)
    return render_template('ops_gmc_import.html', active_ops_page='gmc')
...
@app.route('/georgia/send-otp', methods=['POST'])
def georgia_send_otp():
    data = request.get_json() or {}
    phone = data.get('phone', '').strip()
    phone_clean = ''.join(c for c in phone if c.isdigit())
    if phone_clean.startswith('91') and len(phone_clean) > 10:
    ...
    last_time = session.get('ge_otp_time')
    if last_time:
        try:
            elapsed = (datetime.now() - datetime.fromisoformat(last_time)).total_seconds()
            if elapsed < 60:
    ...
    otp_code = str(random.randint(100000, 999999))
    session['ge_otp'] = otp_code
    session['ge_otp_phone'] = phone_clean
    session['ge_otp_time'] = datetime.now().isoformat()
    ...
    try:
        ...
        logging.error(f"Georgia OTP send error: {e}")
        return jsonify({'error': 'Failed to send OTP. Please try again.'}), 500


@app.route('/georgia/verify-otp', methods=['POST'])
def georgia_verify_otp():
    data = request.get_json() or {}
    entered_otp = data.get('otp', '').strip()
    if not entered_otp:
        return jsonify({'error': 'Please enter the OTP'}), 400

    stored_otp = session.get('ge_otp')
    stored_time = session.get('ge_otp_time')
    if not stored_otp:
        return jsonify({'error': 'OTP expired. Please request a new one.'}), 400

    try:
        if (datetime.now() - datetime.fromisoformat(stored_time)).total_seconds() > 600:
            session.pop('ge_otp', None)
            return jsonify({'error': 'OTP expired. Please request a new one.'}), 400
    except Exception:
        pass
    ...
    session.pop('ge_otp', None)
    session['ge_phone_verified'] = session.get('ge_otp_phone')
    return jsonify({'success': True, 'message': 'Phone number verified successfully'})


# ═══════════════════════════════════════════════════════════════════════════════
# RUSSIA LANDING PAGE (public, no login required)
# ═══════════════════════════════════════════════════════════════════════════════

@app.route('/russia')
def russia_landing():
    return render_template('russia_landing.html')


@app.route('/russia/send-otp', methods=['POST'])
def russia_send_otp():
    data = request.get_json() or {}
    phone = data.get('phone', '').strip()
    phone_clean = ''.join(c for c in phone if c.isdigit())
    ...
    last_time = session.get('ru_otp_time')
    if last_time:
        try:
            elapsed = (datetime.now() - datetime.fromisoformat(last_time)).total_seconds()
            if elapsed < 60:
    ...
    otp_code = str(random.randint(100000, 999999))
    session['ru_otp'] = otp_code
    session['ru_otp_phone'] = phone_clean
    session['ru_otp_time'] = datetime.now().isoformat()
    ...
    try:
        ...
        logging.error(f"Russia OTP send error: {e}")
        return jsonify({'error': 'Failed to send OTP. Please try again.'}), 500


@app.route('/russia/verify-otp', methods=['POST'])
def russia_verify_otp():
    data = request.get_json() or {}
    entered_otp = data.get('otp', '').strip()
    if not entered_otp:
        return jsonify({'error': 'Please enter the OTP'}), 400

    stored_otp = session.get('ru_otp')
    stored_time = session.get('ru_otp_time')
    if not stored_otp:
        return jsonify({'error': 'OTP expired. Please request a new one.'}), 400

    try:
        if (datetime.now() - datetime.fromisoformat(stored_time)).total_seconds() > 600:
            session.pop('ru_otp', None)
            return jsonify({'error': 'OTP expired. Please request a new one.'}), 400
    except Exception:
        pass
    ...
    session.pop('ru_otp', None)
    session['ru_phone_verified'] = session.get('ru_otp_phone')
    return jsonify({'success': True, 'message': 'Phone number verified successfully'})
