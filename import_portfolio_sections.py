"""
import_portfolio_sections.py — import Portfolio pathway data from the new
two-sheet Zoho exports (pathway='portfolio').

Order matters:
  1. step_clients()  — upsert plab_clients (pathway='portfolio') from the
     Registration list. Portfolio currently has ZERO clients, so this CREATES
     them (with their GCPPLUS registration numbers from the file).
  2. step_sections() — replace-per-section the ops_* rows for pathway='portfolio',
     reg-matched to those clients.

Reuses the battle-tested helpers + reg-matching from import_amc_sections.

Run:
  DATABASE_URL=...  DRY_RUN=1  python3 import_portfolio_sections.py   # plan only
  DATABASE_URL=...               python3 import_portfolio_sections.py   # live
  STEP=clients|sections (default all)
"""
import os
...
DL = os.environ.get('PF_DIR', '/Users/Santosh/Desktop/Zoho Data/Portfolio Services')
PATHWAY = 'portfolio'
DRY = os.environ.get('DRY_RUN') in ('1', 'true', 'True')


def _resolve(fname):
    for cand in (fname, f"Portfolio - {fname}", f"Protfolio - {fname}"):
...
# ── STEP: clients ──────────────────────────────────────────────────
CLIENT_FILE = "Portfolio - Registration list.xlsx"
CLIENT_MAP = {
    ...
    'package_amount': ('Package (Without GST)', 'num'),
    'discount_allowed': ('Discount Amount', 'num'),
    'final_package': ('Final Package', 'num'),
    'counsellor': ('Counsellor', 's'), 'account_status': ('Account Status', 's'),
    'career_stage': ('Career Stage', 's'),
    ...
    'additional_package_notes': ('Discount Offered Reason', 's'),
...
def step_clients():
    ...
    cur.execute("SELECT registration_number FROM plab_clients WHERE COALESCE(pathway,'plab')='portfolio'")
...
def build_index():
    ...
    cur.execute("SELECT registration_number, first_name, last_name, email, mobile "
                "FROM plab_clients WHERE COALESCE(pathway,'plab')='portfolio'")
...
# ── SECTIONS (replace-per-section) ─────────────────────────────────
# (file, table, reg_col, name_col, mapping{table_col:(excel_header,kind)})
SECTIONS = {
 'payments': ("Portfolio - All Payments Report.xlsx", "ops_payments", "Registration Number", "Enter Candidate Name", {
 ...
 'research': ("Portfolio - Research and Publication.xlsx", "ops_research_publication", None, "Enter Candidate Name", {
    'service': ('Service', 's'), 'research_start_date': ('Research Start Date', 'd'),
    'research_provider': ('Research Provider', 's'), 'research_batch': ('Research Batch', 's'),
    'research_topic': ('Research Topic', 's'), 'author_position': ('Author Position', 's'),
    'research_status': ('Research Status', 's'), 'additional_notes': ('Notes', 's'),
 }),
 'courses': ("Protfolio - Courses and Certifications Report.xlsx", "ops_online_courses", None, "Enter Candidate Name", {
    'courses_name': ('Course Name', 's'), 'course_type': ('Course Type', 's'),
    'start_date': ('Date', 'd'), 'course_provider': ('Course Provider', 's'),
    'course_status': ('Course Status', 's'),
 }),
 'webinars': ("Protfolio - Webinars & Conferences Report.xlsx", "ops_webinars_conferences", None, "Candidate Name", {
    'provider': ('Provider', 's'), 'start_date': ('Start Date', 'd'),
    'participation_type': ('Participation Type', 's'), 'event_status': ('Event Status', 's'),
    'notes': ('Notes', 's'), 'event_type': ('Event Type', 's'),
    'event_name': ('Webinar Name', 's'), 'event_value': ('Event Value', 's'),
 }),
 'call_notes': ("Protfolio - Client Call Notes Report (1).xlsx", "ops_call_notes", None, "Enter Candidate Name", {
    'call_date': ('Call Date', 'd'), 'call_note': ('Call Note', 's'),
 }),
}
...
def step_sections():
    ...
    for key, (fname, table, reg_col, name_col, mapping) in SECTIONS.items():
        ...
        for r in it:
            ...
            row_vals = [reg] + [CONV[k](g(r, h)) for (h, k) in mapping.values()] + ['portfolio']
            batch.append(tuple(row_vals))
        wb.close()
        cols = ['registration_number'] + list(mapping.keys()) + ['pathway']
        if DRY:
            print(f"[{key:11}] {table:26} would insert={len(batch):4} unmatched={unmatched}")
            continue
        conn = _pg(); conn.autocommit = False; cur = conn.cursor()
        try:
            cur.execute(f"DELETE FROM {table} WHERE COALESCE(pathway,'plab')='portfolio'")
            if batch:
            ...
            conn.commit()
            print(f"[{key:11}] {table:26} inserted={len(batch):4} unmatched={unmatched}")
...
def run():
    ...
    print(f"=== Portfolio import  DRY={DRY}  STEP={step} ===")
