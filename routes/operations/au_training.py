...
# ── Editable columns on ops_coaching (pathway='australia' scope) ────────
...
AU_TRAINING_EDITABLE_COLUMNS = [
    ...
    'vendor_provider', 'other_vendor', 'booked_by',
...
@admin_required
def ops_australia_training_detail(rid):
    """Read-only detail view for one Australia training row.
    ...
    try:
        ...
        logging.error(f"ops_australia_training_detail: {e}")
        record = None
    finally:
        conn.close()

    if not record:
        flash('Australia training record not found.', 'error')
        return redirect(url_for('ops_australia_training_list'))
...
@admin_required
def ops_australia_training_edit_page(rid):
    """GET — render the edit form for one Australia training row."""
    user = get_user()
    conn = get_db()
    try:
        record = conn.execute(
        ...
        logging.error(f"ops_australia_training_edit_page: {e}")
        record = None
    finally:
        conn.close()
    ...
    return render_template(
        ...
        **section_training_lookups('australia'),
    )


@admin_required
def ops_australia_training_edit_save(rid):
    """POST — save changes to an Australia training row.
    ...
    try:
        existing = conn.execute(
            "SELECT id FROM ops_coaching WHERE id = ? AND COALESCE(pathway, 'plab') = 'australia'",
            (rid,),
        ).fetchone()
        if not existing:
            flash('Australia training record not found.', 'error')
        ...
        logging.error(f"ops_australia_training_edit_save: {e}")
    ...
    return redirect(url_for('ops_australia_training_detail', rid=rid))


@admin_required
def ops_australia_training_add_page():
...
@admin_required
def ops_australia_training_add_save():
    """POST — insert a new Australia training row, then redirect to detail."""
    conn = get_db()
    try:
        reg = (request.form.get('registration_number') or '').strip()
        if not reg:
            flash('Registration number is required.', 'error')
            return redirect(url_for('ops_australia_training_add_page'))
        ...
        logging.error(f"ops_australia_training_add_save: {e}")
...
def register_routes(app):
    ...
    app.add_url_rule(
        '/operations/australia/training/<int:rid>',
        endpoint='ops_australia_training_detail',
        view_func=ops_australia_training_detail,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/australia/training/add',
        endpoint='ops_australia_training_add_page',
        view_func=ops_australia_training_add_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/australia/training/add',
        endpoint='ops_australia_training_add_save',
        view_func=ops_australia_training_add_save,
        methods=['POST'],
    )
    app.add_url_rule(
        '/operations/australia/training/<int:rid>/edit',
        endpoint='ops_australia_training_edit_page',
        view_func=ops_australia_training_edit_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/australia/training/<int:rid>/edit',
        endpoint='ops_australia_training_edit_save',
        view_func=ops_australia_training_edit_save,
        methods=['POST'],
    )
