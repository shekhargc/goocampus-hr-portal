"""
routes/operations/tr_call_notes.py — Operations: Training Call Notes list.

Surfaces call-note records from ops_call_notes WHERE pathway='training'.
...
# template matches the other Training sections.
TR_CALL_NOTES_NUMERIC_COLUMNS = {
    'amount', 'score', 'points', 'duration',
}


# Stage-based follow-up intervals for Training clients (mirrors PLAB's
# STAGE_FOLLOWUP_DAYS pattern). User can adjust these per Training stage
# semantics via Settings later — for now they match PLAB's defaults so the
# urgency math is consistent across pathways.
TR_STAGE_FOLLOWUP_DAYS = {
    # Common stages — adjust here when Training's stage taxonomy is finalised.
    'English Stage':       7,
    'Discovery Stage':     15,
    'Engagement Stage':    15,
    'AHPRA Stage':         30,
...
TR_STAGE_FOLLOWUP_DEFAULT = 15


def _tr_stage_interval(stage):
    return TR_STAGE_FOLLOWUP_DAYS.get(stage or '', TR_STAGE_FOLLOWUP_DEFAULT)


def _tr_categorise(days_since, stage):
    """Mirror PLAB's categorise_by_stage. Urgency bands:
       never / critical / overdue / attention / ok / recent."""
    interval = _tr_stage_interval(stage)
...
@admin_required
def ops_training_call_notes_list():
    """Training call notes list — Notes tab.
    ...
    if (not request.args.get('tab')
        ...
        return redirect(url_for('ops_training_call_notes_tracker'))
    ...
    try:
        sql_base = '''SELECT n.id, n.registration_number, n.call_date, n.call_note,
                        ...
                        LEFT JOIN plab_clients p
                              ...
                              AND COALESCE(p.pathway, 'plab') = 'training'
                       WHERE COALESCE(n.pathway, 'plab') = 'training' '''
        params = []
        if reg:
        ...
        logging.error(f"ops_training_call_notes_list: {e}")
        flash(f'Error loading Training call notes: {e}', 'error')
    finally:
        try: conn.close()
    ...
    return render_template(
        'ops_training_call_notes_list.html',
        user=user,
        # Notes tab data
        records=records,
        ...
        stage_filter='',
        stage_counts={},
        stage_intervals=TR_STAGE_FOLLOWUP_DAYS,
        # Shared
        pathway_name='Training Pathway',
        active_tab='notes',
        active_ops_page='training-call-notes',
        active_pathway='training',
    )


@admin_required
def ops_training_call_notes_tracker():
    """Training Follow-up Tracker — mirrors PLAB ops_call_notes_tracker.
    ...
    try:
        sql = '''
         ...
         LEFT JOIN ops_call_notes n
               ...
               AND COALESCE(n.pathway, 'plab') = 'training'
             WHERE COALESCE(p.pathway, 'plab') = 'training'
        ...
        for row in rows:
            ...
            if not last_call_date or contacted_count == 0:
                ...
                if last_call:
                    ...
                    category = _tr_categorise(days_since, stage)
                else:
                    category = 'never'
        ...
        logging.error(f"ops_training_call_notes_tracker: {e}")
    finally:
        try: conn.close()
        except Exception: pass
    ...
    return render_template(
        ...
        stage_counts=stage_counts, stage_intervals=TR_STAGE_FOLLOWUP_DAYS,
...
@admin_required
def ops_training_call_notes_not_contacted():
    """Training Not Contacted Tracker — mirrors PLAB ops_call_notes_not_contacted.

    Lists Training clients with at least one contacted_status='No' note,
    ...
    try:
             ...
             WHERE COALESCE(p.pathway, 'plab') = 'training'
               ...
               AND EXISTS (SELECT 1 FROM ops_call_notes n2
                            WHERE n2.registration_number = p.registration_number
                              AND COALESCE(n2.pathway, 'plab') = 'training'
        ...
        logging.error(f"ops_training_call_notes_not_contacted: {e}")
    finally:
        try: conn.close()
        except Exception: pass
...
@admin_required
def ops_training_call_notes_detail(rid):
    """Full Training call note view — read-only detail card.

    LEFT JOINs plab_clients via registration_number (pathway='training') so
    ...
    conn.close()
    if not record:
        flash('Training call note not found.', 'error')
        return redirect(url_for('ops_training_call_notes_list'))
    return render_template(
        'ops_training_call_notes_detail.html',
        user=user,
        record=record,
        pathway_name='Training Pathway',
        active_ops_page='training-call-notes',
...
@admin_required
def ops_training_call_notes_edit_page(rid):
    """GET — render the edit form for an Training call note."""
    user = get_user()
    conn = get_db()
    record = conn.execute(
        """SELECT n.*, p.first_name, p.last_name, p.prefix
    ...
    if not record:
        ...
        return redirect(url_for('ops_training_call_notes_list'))
    return render_template(
        'ops_training_call_notes_edit.html',
        user=user,
        record=record,
        pathway_name='Training Pathway',
        active_ops_page='training-call-notes',
...
@admin_required
def ops_training_call_notes_edit_save(rid):
    """POST handler: save changes to an Training call note's editable fields.

    Strict allowlist (TR_CALL_NOTES_EDITABLE_COLUMNS). Pathway is NEVER
    ...
    try:
        ...
        sets = []
        params = []
        for col in TR_CALL_NOTES_EDITABLE_COLUMNS:
            if col in request.form:
                val = request.form.get(col, '').strip()
                if col in TR_CALL_NOTES_NUMERIC_COLUMNS:
                    try:
        ...
        if sets:
            ...
            conn.execute(
                ...
                f" WHERE id = ? AND COALESCE(pathway, 'plab') = 'training'",
                params,
            )
            conn.commit()
        ...
        logging.error(f"ops_training_call_notes_edit_save: {e}")
    ...
    return redirect(url_for('ops_training_call_notes_detail', rid=rid))


# ── ADD form (mirrors PLAB's /operations/call-notes/add) ────────────────
@admin_required
def ops_training_call_notes_add():
    """GET/POST: Create a new Training call note.

    Mirrors the PLAB call notes form structure: Client autocomplete (scoped
    to pathway='training'), Contacted Yes/No, Contact Type
    (Call/WhatsApp/Email — hidden when Contacted=No), Date (defaults today),
    Added By, Note. New row written with pathway='training'.
    """
    user = get_user()
    if request.method == 'POST':
        ...
        if not registration_number:
            ...
            return redirect(url_for('ops_training_call_notes_add'))
        if not call_date:
        ...
        try:
            client = conn.execute(
                ...
                "WHERE registration_number = ? AND COALESCE(pathway, 'plab') = 'training'",
                (registration_number,),
            ).fetchone()
            if not client:
                flash('Selected client is not an Training Pathway client.', 'error')
                try: conn.rollback()
                except Exception: pass
                return redirect(url_for('ops_training_call_notes_add'))
            ...
            logging.error(f"ops_training_call_notes_add: {e}")
            try: conn.rollback()
            except Exception: pass
    ...
    return render_template(
        'ops_training_call_notes_add.html',
        user=user,
        pre_reg=pre_reg,
...
def register_routes(app):
    ...
    app.add_url_rule(
        '/operations/training/call-notes',
        endpoint='ops_training_call_notes_list',
        view_func=ops_training_call_notes_list,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/training/call-notes/<int:rid>',
        endpoint='ops_training_call_notes_detail',
        view_func=ops_training_call_notes_detail,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/training/call-notes/<int:rid>/edit',
        endpoint='ops_training_call_notes_edit_page',
        view_func=ops_training_call_notes_edit_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/training/call-notes/<int:rid>/edit',
        endpoint='ops_training_call_notes_edit_save',
        view_func=ops_training_call_notes_edit_save,
        methods=['POST'],
    )
    # Add form — mirrors PLAB /operations/call-notes/add structure.
    app.add_url_rule(
        '/operations/training/call-notes/add',
        endpoint='ops_training_call_notes_add',
        view_func=ops_training_call_notes_add,
        methods=['GET', 'POST'],
    )
    # Follow-up Tracker tab — categorises Training "In Process" clients by
    # contact urgency, just like PLAB /operations/call-notes/tracker.
    app.add_url_rule(
        '/operations/training/call-notes/tracker',
        endpoint='ops_training_call_notes_tracker',
        view_func=ops_training_call_notes_tracker,
        methods=['GET'],
    )
    # Not Contacted tab — Training clients with contacted_status='No' notes.
    app.add_url_rule(
        '/operations/training/call-notes/not-contacted',
        endpoint='ops_training_call_notes_not_contacted',
        view_func=ops_training_call_notes_not_contacted,
        methods=['GET'],
    )
