"""
routes/operations/pf_mentorship.py — Operations: Portfolio Mentorship.

S-2c. Clone of the PLAB Mentorship handlers (ops_mentorship_* in app.py)
scoped to pathway='portfolio'. The underlying ops_mentorship table is
shared across pathways; the `pathway` column was added by the Phase-1
foundation migration.

Endpoints registered:
    GET  /operations/portfolio/mentorship
    GET  /operations/portfolio/mentorship/add        (form)
    POST /operations/portfolio/mentorship/add        (submit)
    GET  /operations/portfolio/mentorship/<int:rid>/edit
    POST /operations/portfolio/mentorship/<int:rid>/edit
    POST /operations/portfolio/mentorship/<int:rid>/delete
"""
...
PF_MENTORSHIP_EXTRA_COLUMNS = ['service', 'fee_currency', 'service_description']
...
def _mentorship_extra_save(conn, rid):
    ...
    sets, params = [], []
    for col in PF_MENTORSHIP_EXTRA_COLUMNS:
...
def _mentorship_lookups():
    ...
    Portfolio) with lookup fallback; mentorship_services is the
    deliverable list the cascade narrows from.
    """
    return {
        'program_providers':  vendor_names_for('Mentorship', 'portfolio',
                                               fallback_lookup='program_provider'),
        'mentorship_services': get_lookup_options('mentorship_service', pathway='portfolio'),
    }
...
def _get_lookup_options(category):
    ...
    try:
        rows = conn.execute(
            ...
            "WHERE category = ? AND COALESCE(pathway, 'plab') = 'portfolio' "
            "  AND is_active = TRUE "
...
@admin_required
def ops_portfolio_mentorship_list():
    ...
    try:
        sql = """SELECT m.*, p.first_name, p.last_name, p.prefix
              ...
              LEFT JOIN plab_clients p ON m.registration_number = p.registration_number
                  WHERE COALESCE(m.pathway, 'plab') = 'portfolio' """
        params = []
        if search:
        ...
        logging.error(f"ops_portfolio_mentorship_list: {e}")
        records = []
    conn.close()
    return render_template(
        'ops_portfolio_mentorship_list.html',
        records=records, search=search,
        active_ops_page='portfolio-mentorship',
        active_pathway='portfolio',
    )


@admin_required
def ops_portfolio_mentorship_add():
    conn = get_db()
        ...
        try:
            cur = conn.execute(
                ...
                (
                    ...
                    'portfolio', session.get('user_id', 0),
                ),
            )
            ...
            logging.error(f"ops_portfolio_mentorship_add: {e}")
            flash(f'Add failed: {e}', 'error')
        conn.close()
        return redirect(request.args.get('next') or url_for('ops_portfolio_mentorship_list'))
    conn.close()
    pre_reg = request.args.get('client', '')
    return render_template(
        'ops_portfolio_mentorship_form.html', record=None,
...
@admin_required
def ops_portfolio_mentorship_edit(rid):
    conn = get_db()
    record = conn.execute(
        ...
        LEFT JOIN plab_clients p
            ...
            WHERE t.id = ?
              AND COALESCE(t.pathway, 'plab') = 'portfolio' """,
        (rid,),
    ).fetchone()
    if not record:
        conn.close()
        flash('Record not found', 'error')
        return redirect(url_for('ops_portfolio_mentorship_list'))
    if request.method == 'POST':
        f = request.form
        try:
            conn.execute(
            ...
            logging.error(f"ops_portfolio_mentorship_edit: {e}")
            flash(f'Update failed: {e}', 'error')
        conn.close()
        return redirect(request.args.get('next') or url_for('ops_portfolio_mentorship_list'))
    conn.close()
    return render_template(
        'ops_portfolio_mentorship_form.html', record=record,
        payment_statuses=_get_lookup_options('mentorship_payment_status'),
        attendance_options=_get_lookup_options('mentorship_attendance'),
        confirmation_options=_get_lookup_options('mentorship_confirmation'),
        pre_reg='',
...
@admin_required
def ops_portfolio_mentorship_detail(rid):
    ...
    Pathway-scoped to portfolio so PLAB / Australia rows are not
    ...
    try:
        record = conn.execute(
            ...
            LEFT JOIN plab_clients p
                  ...
                  AND COALESCE(p.pathway, 'plab') = 'portfolio'
                WHERE m.id = ?
                  AND COALESCE(m.pathway, 'plab') = 'portfolio' """,
            (rid,),
        ).fetchone()
    except Exception as e:
        logging.error(f"ops_portfolio_mentorship_detail: {e}")
        record = None
    finally:
        conn.close()
    ...
    return render_template(
        'ops_portfolio_mentorship_detail.html',
        user=user,
        record=record,
        pathway_name='Portfolio',
        active_ops_page='portfolio-mentorship',
        active_pathway='portfolio',
    )
...
@admin_required
def ops_portfolio_mentorship_delete(rid):
    conn = get_db()
    try:
        conn.execute(
        ...
        logging.error(f"ops_portfolio_mentorship_delete: {e}")
...
def register_routes(app):
    app.add_url_rule(
        '/operations/portfolio/mentorship',
        endpoint='ops_portfolio_mentorship_list',
        view_func=ops_portfolio_mentorship_list,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/portfolio/mentorship/add',
        endpoint='ops_portfolio_mentorship_add',
        view_func=ops_portfolio_mentorship_add,
        methods=['GET', 'POST'],
    )
    app.add_url_rule(
        '/operations/portfolio/mentorship/<int:rid>',
        endpoint='ops_portfolio_mentorship_detail',
        view_func=ops_portfolio_mentorship_detail,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/portfolio/mentorship/<int:rid>/edit',
        endpoint='ops_portfolio_mentorship_edit',
        view_func=ops_portfolio_mentorship_edit,
        methods=['GET', 'POST'],
    )
    app.add_url_rule(
        '/operations/portfolio/mentorship/<int:rid>/delete',
        endpoint='ops_portfolio_mentorship_delete',
        view_func=ops_portfolio_mentorship_delete,
        methods=['POST'],
    )
