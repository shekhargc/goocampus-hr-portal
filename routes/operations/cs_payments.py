"""
routes/operations/cs_payments.py — Standard Consulting: Payments list + detail + edit.

Surfaces rows from ops_payments WHERE pathway='consulting', joined to
plab_clients on registration_number so each row shows the candidate's
human name alongside the payment record.

Endpoint name: ops_consulting_payments_list. Sidebar pathway switcher
links to /operations/consulting/payments.

Detail + edit routes mirror the consulting Registration pattern
(ops_consulting_clients_list / _edit_page / _edit_save) so the UX is
identical across every consulting section.
"""
...
CS_PAYMENTS_NUMERIC_COLUMNS = {
    'amount_paid', 'gst_paid', 'total_amount_paid', 'total_package',
}


@admin_required
def ops_consulting_payments_list():
    """Australia payments list — ops_payments WHERE pathway='consulting'."""
    user = get_user()
    conn = get_db()
        ...
        instalments = [
            r['instalment'] for r in conn.execute(
                """SELECT DISTINCT instalment FROM ops_payments
                    WHERE pathway = 'consulting' AND instalment IS NOT NULL AND instalment != ''
                    ORDER BY instalment"""
            ).fetchall()
        ]
        methods = [
            r['payment_method'] for r in conn.execute(
                """SELECT DISTINCT payment_method FROM ops_payments
                    WHERE pathway = 'consulting' AND payment_method IS NOT NULL AND payment_method != ''
                    ORDER BY payment_method"""
            ).fetchall()
        ]
    except Exception as e:
        logging.error(f"ops_consulting_payments_list: {e}")
        flash(f'Error loading Australia payments: {e}', 'error')
    finally:
        conn.close()

    return render_template(
        'ops_consulting_payments_list.html',
        user=user,
        records=records,
        total=total,
        ...
        active_ops_page='consulting-payments',
        active_pathway='consulting',
    )


@admin_required
def ops_consulting_payments_detail(rid):
    ...
    if not record:
        ...
        return redirect(url_for('ops_consulting_payments_list'))

    return render_template(
        'ops_consulting_payments_detail.html',
        user=user,
        record=record,
        pathway_name='Standard Consulting',
        active_ops_page='consulting-payments',
...
@admin_required
def ops_consulting_payments_edit_page(rid):
    ...
    return render_template(
        'ops_consulting_payments_edit.html',
        user=user,
        record=record,
        pathway_name='Standard Consulting',
        active_ops_page='consulting-payments',
        ...
        **section_payment_lookups('consulting'),
    )


@admin_required
def ops_consulting_payments_edit_save(rid):
    """POST — save edits to an Australia payment row.

    Strict allowlist (CS_PAYMENTS_EDITABLE_COLUMNS) — anything else in the
    ...
    try:
        ...
        sets = []
        params = []
        for col in CS_PAYMENTS_EDITABLE_COLUMNS:
            if col in request.form:
                val = request.form.get(col, '').strip()
                if col in CS_PAYMENTS_NUMERIC_COLUMNS:
                    try:
        ...
        logging.error(f"ops_consulting_payments_edit_save: {e}")
    ...
    return redirect(url_for('ops_consulting_payments_detail', rid=rid))


def register_routes(app):
    """Attach this sub-area's URL rules to the Flask app."""
    app.add_url_rule(
        '/operations/consulting/payments',
        endpoint='ops_consulting_payments_list',
        view_func=ops_consulting_payments_list,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/payments/<int:rid>',
        endpoint='ops_consulting_payments_detail',
        view_func=ops_consulting_payments_detail,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/payments/<int:rid>/edit',
        endpoint='ops_consulting_payments_edit_page',
        view_func=ops_consulting_payments_edit_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/payments/<int:rid>/edit',
        endpoint='ops_consulting_payments_edit_save',
        view_func=ops_consulting_payments_edit_save,
        methods=['POST'],
    )
