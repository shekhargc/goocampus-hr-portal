"""
routes/operations/au_amc.py — Operations: AMC Pathway AMC Registration.
...
next to ops_amc_registration. scoped to pathway='australia'. The underlying table
ops_amc_registration has had a pathway column since the Phase-1 foundation
migration, so australia rows live alongside PLAB rows in the same table.

Endpoints registered:
    GET  /operations/australia/amc
    GET  /operations/australia/amc/add        (form)
    POST /operations/australia/amc/add        (submit)
    GET  /operations/australia/amc/<int:gid>/edit
    POST /operations/australia/amc/<int:gid>/edit
    POST /operations/australia/amc/<int:gid>/delete
"""
...
def _get_lookup_options(category):
    ...
    load. Falls back to the PLAB-pathway lookups when no australia
...
@admin_required
def ops_australia_amc_list():
    """AMC Pathway AMC list. Mirror of ops_gmc_list / cs_gmc (new schema) filtered to
    pathway='australia'."""
    conn = get_db()
    ...
    try:
        sql = """SELECT g.*, p.first_name, p.last_name, p.prefix
              ...
              LEFT JOIN plab_clients p ON g.registration_number = p.registration_number
                  WHERE COALESCE(g.pathway, 'plab') = 'australia' """
        params = []
        if status_filter:
            sql += " AND g.amc_setup = ? "
        ...
        logging.error(f"ops_australia_amc_list: {e}")
        records = []
    conn.close()
    return render_template(
        'ops_australia_amc_list.html',
        records=records,
...
@admin_required
def ops_australia_amc_add():
    """Add an AMC pathway AMC registration record."""
    conn = get_db()
    if request.method == 'POST':
        ...
        try:
            ...
            logging.error(f"ops_australia_amc_add: {e}")
            flash(f'Add failed: {e}', 'error')
        conn.close()
        return redirect(request.args.get('next') or url_for('ops_australia_amc_list'))
...
@admin_required
def ops_australia_amc_edit(gid):
    """Edit an AMC pathway AMC registration record. Restricted to
    rows where pathway='australia' so PLAB / AMC data can't be
    mutated through this surface."""
    conn = get_db()
    record = conn.execute(
        "SELECT * FROM ops_amc_registration "
    ...
    if not record:
        ...
        return redirect(url_for('ops_australia_amc_list'))
    if request.method == 'POST':
        f = request.form
        try:
            conn.execute(
            ...
            logging.error(f"ops_australia_amc_edit: {e}")
...
@admin_required
def ops_australia_amc_detail(gid):
    """Read-only detail view for one AMC Pathway AMC registration row.
    ...
    Pathway-scoped to australia so PLAB / Consulting rows are not
    ...
    try:
        record = conn.execute(
            ...
            LEFT JOIN plab_clients p
                ...
                WHERE g.id = ?
                  AND COALESCE(g.pathway, 'plab') = 'australia' """,
            (gid,),
        ).fetchone()
    except Exception as e:
        logging.error(f"ops_australia_amc_detail: {e}")
        record = None
    finally:
        conn.close()
...
@admin_required
def ops_australia_amc_delete(gid):
    """Delete an AMC pathway AMC registration record. Pathway gated."""
    conn = get_db()
    try:
        conn.execute(
        ...
        logging.error(f"ops_australia_amc_delete: {e}")
...
def register_routes(app):
    """Register AMC Pathway AMC routes."""
    app.add_url_rule(
        '/operations/australia/amc',
        endpoint='ops_australia_amc_list',
        view_func=ops_australia_amc_list,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/australia/amc/add',
        endpoint='ops_australia_amc_add',
        view_func=ops_australia_amc_add,
        methods=['GET', 'POST'],
    )
    app.add_url_rule(
        '/operations/australia/amc/<int:gid>',
        endpoint='ops_australia_amc_detail',
        view_func=ops_australia_amc_detail,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/australia/amc/<int:gid>/edit',
        endpoint='ops_australia_amc_edit',
        view_func=ops_australia_amc_edit,
        methods=['GET', 'POST'],
    )
    app.add_url_rule(
        '/operations/australia/amc/<int:gid>/delete',
        endpoint='ops_australia_amc_delete',
        view_func=ops_australia_amc_delete,
        methods=['POST'],
    )
