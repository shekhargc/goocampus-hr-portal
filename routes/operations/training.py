"""
routes/operations/training.py — Operations: Training Pathway.

Lightweight pathway cloned from Standard Consulting. Surfaces basic stats
and a single clients list for the Training Pathway, plus Payments,
Documents and Call Notes sections only (no medical sections).

Data model
----------
Clients live in `plab_clients` with pathway='training'. Products live in
products_services with pathway='training'. The product a client signed up
for is captured at invitation time in client_invitations.product_id and
carried through to client_registrations.product_id, then to
plab_clients.product_id.

URL space
---------
    GET /operations/training               -> dashboard
    GET /operations/training/clients       -> clients list

Endpoint names
--------------
    ops_training_pathway        (dashboard view)
    ops_training_clients_list   (clients list)
"""
...
# Pathway-scoped dropdown options for the Training Registration edit form.
from routes.operations._form_lookups import section_client_lookups, section_client_products, transfer_for_reg


@admin_required
def ops_training_pathway():
    """Training Pathway dashboard -- mirrors the Australia dashboard
    ...
    stats = {
        ...
        'this_fy_new':          0,
        # Per-product split -- the user-visible "what training type"
    ...
    try:
        ...
        stats['total_clients'] = conn.execute(
            "SELECT COUNT(*) AS c FROM plab_clients WHERE pathway = 'training'"
        ).fetchone()['c']

        for row in conn.execute(
            """SELECT COALESCE(NULLIF(TRIM(account_status), ''), 'Unknown') AS s,
                      COUNT(*) AS c
               FROM plab_clients WHERE pathway = 'training'
        ...
        for row in conn.execute(
            """SELECT COALESCE(NULLIF(TRIM(current_stage), ''), 'Not Set') AS stg,
                      COUNT(*) AS c
               FROM plab_clients
               WHERE pathway = 'training' AND account_status = 'In Process'
        ...
        # ── This-FY new registrations (reg-number prefix GCTRN/<FY>/) ──
        ...
        stats['this_fy_new'] = conn.execute(
            ...
            "WHERE pathway = 'training' AND registration_number LIKE ?",
            (f'GCTRN/{fy}/%',),
        ).fetchone()['c']

        # ── X-4b: AMC-style section card counts (most are 0 for
        # training -- those sections show 0 in the section grid). ──
        CS = "COALESCE(pathway, 'plab') = 'training'"
        def cnt(sql, *params):
            try:
        ...
        # Sections that AMC has but training doesn't -- 0 stub so the
        ...
        try:
            ...
            stats['recent_registrations'] = conn.execute(
                """SELECT id, registration_number, prefix, first_name,
                          ...
                          registration_date
                   FROM plab_clients
                   WHERE pathway = 'training'
                   ORDER BY id DESC
                   LIMIT 5"""
            ).fetchall()

    except Exception as e:
        logging.error(f"ops_training_pathway: {e}")
        flash(f'Error loading Training Pathway dashboard: {e}', 'error')
    finally:
        conn.close()

    return render_template(
        'ops_training_pathway_dashboard.html',
        user=user,
        pathway_name='Training Pathway',
        stats=stats,
        active_ops_page='training',
        active_pathway='training',
    )


@admin_required
def ops_training_clients_list():
    """Training Pathway clients list -- matches the AMC list template
    ...
    try:
        ...
        logging.error(f"ops_training_clients_list: {e}")
        flash(f'Error loading clients list: {e}', 'error')
    finally:
        conn.close()

    return render_template(
        'ops_training_clients.html',
        user=user,
        records=records,
        total=total,
        search=search,
        ...
        active_ops_page='training-clients',
        active_pathway='training',
    )
...
def _cs_payment_totals_for(conn, reg_num):
    """Sum (amount_paid, gst_paid, total_paid) for ONE training client
...
@admin_required
def ops_training_client_detail(client_id):
    """Full training client profile -- view + linked-section panels.

    Mirror of ops_consulting_client_detail scoped to pathway='training'.
    Lightweight: only fetches Payments and Call Notes; all medical
    sections are stubbed empty.
    """
    user = get_user()
    conn = get_db()
    client = conn.execute(
        "SELECT * FROM plab_clients "
        " WHERE id = ? AND COALESCE(pathway, 'plab') = 'training'",
        (client_id,),
    ).fetchone()
    if not client:
        conn.close()
        flash('Training client not found.', 'error')
        return redirect(url_for('ops_training_clients_list'))
    ...
    def fetch(table, order=None):
        try:
            sql = (
                ...
                f"  AND COALESCE(pathway, 'plab') = 'training' "
            )
            if order:
...
@admin_required
def ops_training_client_by_reg(reg):
    """Resolve a Training client by registration_number → detail redirect.

    Used by section drawers (Call Notes, Payments) whose "View Client
    Profile" button only knows the registration_number, not the numeric
    client_id required by /operations/training/clients/<id>.
    """
    conn = get_db()
    row = conn.execute(
        "SELECT id FROM plab_clients WHERE registration_number = ? "
        "AND COALESCE(pathway, 'plab') = 'training' LIMIT 1",
        (reg,)
    ).fetchone()
    conn.close()
    if not row:
        flash(f'Training client {reg} not found.', 'error')
        return redirect(url_for('ops_training_clients_list'))
    return redirect(url_for('ops_training_client_detail', client_id=row['id']))


@admin_required
def ops_training_client_edit_page(client_id):
    """GET -- render the edit form for a training client."""
    user = get_user()
    conn = get_db()
    client = conn.execute(
        "SELECT * FROM plab_clients "
    ...
    if not client:
        ...
        return redirect(url_for('ops_training_clients_list'))
    return render_template(
        'ops_training_client_edit_form.html',
        user=user,
        client=client,
        transfer=xfer,
        pathway_name='Training Pathway',
...
@admin_required
def ops_training_client_edit_save(client_id):
    """POST -- save edits to a training client's editable fields."""
    conn = get_db()
    try:
        ...
        logging.error(f"ops_training_client_edit_save: {e}")
        flash(f'Error saving client: {e}', 'error')
        try: conn.rollback()
        except Exception: pass
    finally:
        conn.close()
    return redirect(url_for('ops_training_client_detail', client_id=client_id))


@admin_required
def ops_training_documents_list():
    """Y-2: Documents overview for training clients. Clone of
    ops_documents_list scoped to pathway='training'. The shared
    ...
    # training checklist is identical until product-specific doc
    ...
    try:
        ...
        logging.error(f"ops_training_documents_list: {e}")
        all_clients = []
    finally:
        conn.close()
    ...
    return render_template(
        'ops_training_documents_list.html',
        user=user,
        ...
        active_ops_page='training-documents',
        active_pathway='training',
    )


@admin_required
def ops_training_client_delete(client_id):
    """Hard-delete a training client (pathway-gated)."""
    conn = get_db()
    try:
        existing = conn.execute(
            "SELECT id FROM plab_clients "
        ...
        logging.error(f"ops_training_client_delete: {e}")
...
def register_routes(app):
    """Register Training Pathway routes with the Flask app."""
    app.add_url_rule(
        '/operations/training',
        endpoint='ops_training_pathway',
        view_func=ops_training_pathway,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/training/clients',
        endpoint='ops_training_clients_list',
        view_func=ops_training_clients_list,
        methods=['GET'],
    )
    # X-4a: per-client detail + edit + delete
    app.add_url_rule(
        '/operations/training/clients/<int:client_id>',
        endpoint='ops_training_client_detail',
        view_func=ops_training_client_detail,
        methods=['GET'],
    )
    # Lookup-by-registration-number helper for section drawers
    app.add_url_rule(
        '/operations/training/clients/by-reg/<path:reg>',
        endpoint='ops_training_client_by_reg',
        view_func=ops_training_client_by_reg,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/training/clients/<int:client_id>/edit',
        endpoint='ops_training_client_edit_page',
        view_func=ops_training_client_edit_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/training/clients/<int:client_id>/edit',
        endpoint='ops_training_client_edit_save',
        view_func=ops_training_client_edit_save,
        methods=['POST'],
    )
    app.add_url_rule(
        '/operations/training/clients/<int:client_id>/delete',
        endpoint='ops_training_client_delete',
        view_func=ops_training_client_delete,
        methods=['POST'],
    )
    # Y-2: documents list
    app.add_url_rule(
        '/operations/training/documents',
        endpoint='ops_training_documents_list',
        view_func=ops_training_documents_list,
        methods=['GET'],
    )
