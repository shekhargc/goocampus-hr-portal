"""
routes/operations/au_webinars.py — Operations: Standard Consulting
Webinars & Conferences list + detail + edit.

Surfaces rows from ops_webinars_conferences WHERE pathway='consulting',
joined to plab_clients on registration_number so we can show the
candidate name. Records are imported one-time from
All_Consulting_Webinars.xlsx by
import_consulting_webinars.run_import_consulting_webinars_once() at app
boot.

Standardised to the PLAB / Consulting Training pattern: list (with
...
# ── Editable columns on ops_webinars_conferences (pathway='consulting') ─
...
@admin_required
def ops_consulting_webinars_list():
    """Consulting webinars & conferences list — ops_webinars_conferences WHERE pathway='consulting'."""
    user = get_user()
    conn = get_db()
        ...
        event_types = [
            r['event_type'] for r in conn.execute(
                """SELECT DISTINCT event_type FROM ops_webinars_conferences
                    WHERE pathway = 'consulting' AND event_type IS NOT NULL AND event_type != ''
                    ORDER BY event_type"""
            ).fetchall()
        ]
        participations = [
            r['participation_type'] for r in conn.execute(
                """SELECT DISTINCT participation_type FROM ops_webinars_conferences
                    WHERE pathway = 'consulting' AND participation_type IS NOT NULL AND participation_type != ''
        ...
        event_values = [
            r['event_value'] for r in conn.execute(
                """SELECT DISTINCT event_value FROM ops_webinars_conferences
                    WHERE pathway = 'consulting' AND event_value IS NOT NULL AND event_value != ''
                    ORDER BY event_value"""
            ).fetchall()
        ]
    except Exception as e:
        logging.error(f"ops_consulting_webinars_list: {e}")
        flash(f'Error loading Consulting webinars & conferences: {e}', 'error')
    finally:
        conn.close()

    return render_template(
        'ops_consulting_webinars_list.html',
        user=user,
        records=records,
        total=total,
        search=search,
        ...
        active_ops_page='consulting-webinars',
        active_pathway='consulting',
    )


@admin_required
def ops_consulting_webinars_detail(rid):
    """Read-only detail view for one Consulting webinar/conference row.
    ...
    try:
        record = conn.execute(
            ...
            LEFT JOIN plab_clients p
                ...
                WHERE w.id = ?
                  AND COALESCE(w.pathway, 'plab') = 'consulting' """,
            (rid,),
        ).fetchone()
    except Exception as e:
        logging.error(f"ops_consulting_webinars_detail: {e}")
        record = None
    finally:
        conn.close()

    if not record:
        flash('Consulting webinar/conference record not found.', 'error')
        return redirect(url_for('ops_consulting_webinars_list'))

    return render_template(
        'ops_consulting_webinars_detail.html',
        user=user,
        record=record,
        pathway_name='Standard Consulting',
        active_ops_page='consulting-webinars',
...
@admin_required
def ops_consulting_webinars_edit_page(rid):
    """GET — render the edit form for one Consulting webinar/conference row."""
    user = get_user()
    conn = get_db()
    try:
        record = conn.execute(
        ...
        logging.error(f"ops_consulting_webinars_edit_page: {e}")
        record = None
    finally:
        conn.close()
    ...
    return render_template(
        'ops_consulting_webinars_edit.html',
        user=user,
        record=record,
        pathway_name='Standard Consulting',
        active_ops_page='consulting-webinars',
        ...
        **section_webinars_lookups('consulting'),
    )


@admin_required
def ops_consulting_webinars_edit_save(rid):
    """POST — save changes to an Consulting webinar/conference row.
    ...
    try:
        existing = conn.execute(
            "SELECT id FROM ops_webinars_conferences WHERE id = ? AND COALESCE(pathway, 'plab') = 'consulting'",
            (rid,),
        ).fetchone()
        if not existing:
            flash('Consulting webinar/conference record not found.', 'error')
        ...
        logging.error(f"ops_consulting_webinars_edit_save: {e}")
    ...
    return redirect(url_for('ops_consulting_webinars_detail', rid=rid))


@admin_required
def ops_consulting_webinars_add_page():
...
@admin_required
def ops_consulting_webinars_add_save():
    """POST — insert a new Consulting webinar / conference row."""
    conn = get_db()
    try:
        reg = (request.form.get('registration_number') or '').strip()
        if not reg:
            flash('Registration number is required.', 'error')
            return redirect(url_for('ops_consulting_webinars_add_page'))
        ...
        return redirect(url_for('ops_consulting_webinars_detail', rid=new_id))
    except Exception as e:
        logging.error(f"ops_consulting_webinars_add_save: {e}")
...
def register_routes(app):
    ...
    app.add_url_rule(
        '/operations/consulting/webinars',
        endpoint='ops_consulting_webinars_list',
        view_func=ops_consulting_webinars_list,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/webinars/<int:rid>',
        endpoint='ops_consulting_webinars_detail',
        view_func=ops_consulting_webinars_detail,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/webinars/add',
        endpoint='ops_consulting_webinars_add_page',
        view_func=ops_consulting_webinars_add_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/webinars/add',
        endpoint='ops_consulting_webinars_add_save',
        view_func=ops_consulting_webinars_add_save,
        methods=['POST'],
    )
    app.add_url_rule(
        '/operations/consulting/webinars/<int:rid>/edit',
        endpoint='ops_consulting_webinars_edit_page',
        view_func=ops_consulting_webinars_edit_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/webinars/<int:rid>/edit',
        endpoint='ops_consulting_webinars_edit_save',
        view_func=ops_consulting_webinars_edit_save,
        methods=['POST'],
    )
