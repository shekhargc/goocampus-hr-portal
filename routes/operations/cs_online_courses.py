"""
routes/operations/cs_online_courses.py — Operations: Standard Consulting
Online Courses list + detail + edit.

Surfaces rows from ops_online_courses WHERE pathway='consulting', joined
...
standardised to the AMC / Consulting pattern: list (with drawer) + detail
...
Endpoints registered:
  - GET  /operations/consulting/online-courses
  - GET  /operations/consulting/online-courses/<rid>
  - GET  /operations/consulting/online-courses/add        (form)
  - POST /operations/consulting/online-courses/add        (submit)
  - GET  /operations/consulting/online-courses/<rid>/edit
  - POST /operations/consulting/online-courses/<rid>/edit
"""
...
def _course_lookups():
    ...
    lk = {
        ...
        'booked_by_options':    get_lookup_options('course_booked_by',   pathway='consulting'),
...
@admin_required
def ops_consulting_online_courses_list():
    """Consulting online courses list — ops_online_courses WHERE pathway='consulting'."""
    user = get_user()
    conn = get_db()
        ...
        courses = [
            r['courses_name'] for r in conn.execute(
                """SELECT DISTINCT courses_name FROM ops_online_courses
                    WHERE pathway = 'consulting' AND courses_name IS NOT NULL
                      AND courses_name != ''
        ...
        statuses = [
            r['course_status'] for r in conn.execute(
                """SELECT DISTINCT course_status FROM ops_online_courses
                    WHERE pathway = 'consulting' AND course_status IS NOT NULL
                      AND course_status != ''
        ...
        providers = [
            r['course_provider'] for r in conn.execute(
                """SELECT DISTINCT course_provider FROM ops_online_courses
                    WHERE pathway = 'consulting' AND course_provider IS NOT NULL
                      AND course_provider != ''
                    ORDER BY course_provider"""
            ).fetchall()
        ]
    except Exception as e:
        logging.error(f"ops_consulting_online_courses_list: {e}")
        flash(f'Error loading Consulting online courses: {e}', 'error')
    finally:
        conn.close()

    return render_template(
        'ops_consulting_online_courses_list.html',
        user=user,
        records=records,
        total=total,
        search=search,
        ...
        active_ops_page='consulting-online-courses',
        active_pathway='consulting',
    )


@admin_required
def ops_consulting_online_courses_detail(rid):
    """Read-only detail page for one Consulting online course row."""
    user = get_user()
    conn = get_db()
    try:
        record = conn.execute(
            ...
            LEFT JOIN plab_clients p
                ...
                WHERE c.id = ?
                  AND COALESCE(c.pathway, 'plab') = 'consulting' """,
            (rid,),
        ).fetchone()
    except Exception as e:
        logging.error(f"ops_consulting_online_courses_detail: {e}")
        record = None
    finally:
        conn.close()
    ...
    return render_template(
        'ops_consulting_online_courses_detail.html',
        user=user,
        record=record,
        pathway_name='Standard Consulting',
        active_ops_page='consulting-online-courses',
...
@admin_required
def ops_consulting_online_courses_edit_page(rid):
    """GET — render the edit form for one Consulting online course row."""
    user = get_user()
    conn = get_db()
    try:
        record = conn.execute(
        ...
        logging.error(f"ops_consulting_online_courses_edit_page: {e}")
        record = None
    finally:
        conn.close()
    ...
    return render_template(
        'ops_consulting_online_courses_edit.html',
        user=user,
        record=record,
        pathway_name='Standard Consulting',
        active_ops_page='consulting-online-courses',
...
@admin_required
def ops_consulting_online_courses_edit_save(rid):
    """POST — save changes to a Consulting online course row.
    ...
    try:
        ...
        logging.error(f"ops_consulting_online_courses_edit_save: {e}")
    ...
    return redirect(url_for('ops_consulting_online_courses_detail', rid=rid))


@admin_required
def ops_consulting_online_courses_add_page():
...
@admin_required
def ops_consulting_online_courses_add_save():
    """POST — insert a new Consulting online course row, then redirect to detail."""
    conn = get_db()
    try:
        ...
        return redirect(url_for('ops_consulting_online_courses_detail', rid=new_id))
    except Exception as e:
        logging.error(f"ops_consulting_online_courses_add_save: {e}")
...
def register_routes(app):
    ...
    app.add_url_rule(
        '/operations/consulting/online-courses',
        endpoint='ops_consulting_online_courses_list',
        view_func=ops_consulting_online_courses_list,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/online-courses/<int:rid>',
        endpoint='ops_consulting_online_courses_detail',
        view_func=ops_consulting_online_courses_detail,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/online-courses/add',
        endpoint='ops_consulting_online_courses_add_page',
        view_func=ops_consulting_online_courses_add_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/online-courses/add',
        endpoint='ops_consulting_online_courses_add_save',
        view_func=ops_consulting_online_courses_add_save,
        methods=['POST'],
    )
    app.add_url_rule(
        '/operations/consulting/online-courses/<int:rid>/edit',
        endpoint='ops_consulting_online_courses_edit_page',
        view_func=ops_consulting_online_courses_edit_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/online-courses/<int:rid>/edit',
        endpoint='ops_consulting_online_courses_edit_save',
        view_func=ops_consulting_online_courses_edit_save,
        methods=['POST'],
    )
