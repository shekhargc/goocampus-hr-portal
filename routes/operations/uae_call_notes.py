"""
routes/operations/pf_call_notes.py — Operations: UAE Call Notes list.

Surfaces call-note records from ops_call_notes WHERE pathway='uae'.
...
# ── Editable columns on ops_call_notes (pathway='uae' scope) ──────────
...
# template matches the other UAE sections.
PF_CALL_NOTES_NUMERIC_COLUMNS = {
    'amount', 'score', 'points', 'duration',
}


# Stage-based follow-up intervals for UAE clients (mirrors PLAB's
# STAGE_FOLLOWUP_DAYS pattern). User can adjust these per UAE stage
...
PF_STAGE_FOLLOWUP_DAYS = {
    # Common stages — adjust here when UAE's stage taxonomy is finalised.
    'English Stage':       7,
    'Discovery Stage':     15,
    'Engagement Stage':    15,
    'AHPRA Stage':         30,
...
@admin_required
def ops_uae_call_notes_list():
    """UAE call notes list — Notes tab.
    ...
    if (not request.args.get('tab')
        ...
        return redirect(url_for('ops_uae_call_notes_tracker'))
    ...
    try:
        sql_base = '''SELECT n.id, n.registration_number, n.call_date, n.call_note,
                       ...
                       WHERE COALESCE(n.pathway, 'plab') = 'uae' '''
        params = []
        if reg:
        ...
        logging.error(f"ops_uae_call_notes_list: {e}")
        flash(f'Error loading UAE call notes: {e}', 'error')
    finally:
        try: conn.close()
    ...
    return render_template(
        ...
        active_tab='notes',
        active_ops_page='uae-call-notes',
        active_pathway='uae',
    )


@admin_required
def ops_uae_call_notes_tracker():
    """UAE Follow-up Tracker — mirrors PLAB ops_call_notes_tracker.
    ...
    try:
        sql = '''
         ...
         LEFT JOIN ops_call_notes n
               ...
               AND COALESCE(n.pathway, 'plab') = 'uae'
             WHERE COALESCE(p.pathway, 'plab') = 'uae'
        ...
        logging.error(f"ops_uae_call_notes_tracker: {e}")
    finally:
        try: conn.close()
        except Exception: pass
...
@admin_required
def ops_uae_call_notes_not_contacted():
    """UAE Not Contacted Tracker — mirrors PLAB ops_call_notes_not_contacted.

    Lists UAE clients with at least one contacted_status='No' note,
    ...
    try:
             ...
             WHERE COALESCE(p.pathway, 'plab') = 'uae'
               ...
               AND EXISTS (SELECT 1 FROM ops_call_notes n2
                            WHERE n2.registration_number = p.registration_number
                              AND COALESCE(n2.pathway, 'plab') = 'uae'
        ...
        logging.error(f"ops_uae_call_notes_not_contacted: {e}")
    finally:
        try: conn.close()
        except Exception: pass
...
@admin_required
def ops_uae_call_notes_detail(rid):
    """Full UAE call note view — read-only detail card.

    LEFT JOINs plab_clients via registration_number (pathway='uae') so
    ...
    conn.close()
    if not record:
        flash('UAE call note not found.', 'error')
        return redirect(url_for('ops_uae_call_notes_list'))
    return render_template(
        'ops_uae_call_notes_detail.html',
        user=user,
        record=record,
        pathway_name='UAE Pathway',
        active_ops_page='uae-call-notes',
...
@admin_required
def ops_uae_call_notes_edit_page(rid):
    """GET — render the edit form for an UAE call note."""
    user = get_user()
    conn = get_db()
    record = conn.execute(
        """SELECT n.*, p.first_name, p.last_name, p.prefix
...
@admin_required
def ops_uae_call_notes_edit_save(rid):
    """POST handler: save changes to an UAE call note's editable fields.
    ...
    try:
        ...
        logging.error(f"ops_uae_call_notes_edit_save: {e}")
    ...
    return redirect(url_for('ops_uae_call_notes_detail', rid=rid))


# ── ADD form (mirrors PLAB's /operations/call-notes/add) ────────────────
@admin_required
def ops_uae_call_notes_add():
    """GET/POST: Create a new UAE call note.

    Mirrors the PLAB call notes form structure: Client autocomplete (scoped
    to pathway='uae'), Contacted Yes/No, Contact Type
    (Call/WhatsApp/Email — hidden when Contacted=No), Date (defaults today),
    Added By, Note. New row written with pathway='uae'.
    """
    user = get_user()
    if request.method == 'POST':
        ...
        if not registration_number:
            ...
            return redirect(url_for('ops_uae_call_notes_add'))
        if not call_date:
        ...
        try:
            ...
            if not client:
                flash('Selected client is not an UAE Pathway client.', 'error')
                try: conn.rollback()
                except Exception: pass
                return redirect(url_for('ops_uae_call_notes_add'))
            ...
            logging.error(f"ops_uae_call_notes_add: {e}")
            try: conn.rollback()
            except Exception: pass
...
def register_routes(app):
    ...
    app.add_url_rule(
        '/operations/uae/call-notes',
        endpoint='ops_uae_call_notes_list',
        view_func=ops_uae_call_notes_list,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/uae/call-notes/<int:rid>',
        endpoint='ops_uae_call_notes_detail',
        view_func=ops_uae_call_notes_detail,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/uae/call-notes/<int:rid>/edit',
        endpoint='ops_uae_call_notes_edit_page',
        view_func=ops_uae_call_notes_edit_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/uae/call-notes/<int:rid>/edit',
        endpoint='ops_uae_call_notes_edit_save',
        view_func=ops_uae_call_notes_edit_save,
        methods=['POST'],
    )
    # Add form — mirrors PLAB /operations/call-notes/add structure.
    app.add_url_rule(
        '/operations/uae/call-notes/add',
        endpoint='ops_uae_call_notes_add',
        view_func=ops_uae_call_notes_add,
        methods=['GET', 'POST'],
    )
    # Follow-up Tracker tab — categorises UAE "In Process" clients by
    # contact urgency, just like PLAB /operations/call-notes/tracker.
    app.add_url_rule(
        '/operations/uae/call-notes/tracker',
        endpoint='ops_uae_call_notes_tracker',
        view_func=ops_uae_call_notes_tracker,
        methods=['GET'],
    )
    # Not Contacted tab — UAE clients with contacted_status='No' notes.
    app.add_url_rule(
        '/operations/uae/call-notes/not-contacted',
        endpoint='ops_uae_call_notes_not_contacted',
        view_func=ops_uae_call_notes_not_contacted,
        methods=['GET'],
    )
