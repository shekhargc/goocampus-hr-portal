    ...
    try:
        ...
        if sets:
            ...
            try:
                ...
                logging.warning(f"australia academic ops->client sync: {_se}")
            conn.commit()
...
@admin_required
def ops_australia_academic_add_page():
    """GET — render the empty add form (reuses the edit template)."""
    user = get_user()
    empty_record = {col: None for col in AU_ACADEMIC_EDITABLE_COLUMNS}
...
@admin_required
def ops_australia_academic_add_save():
    """POST — insert a new Australia academic-details row."""
    conn = get_db()
    try:
        reg = (request.form.get('registration_number') or '').strip()
        if not reg:
            flash('Registration number is required.', 'error')
            return redirect(url_for('ops_australia_academic_add_page'))
        ...
        return redirect(url_for('ops_australia_academic_detail', rid=new_id))
    except Exception as e:
        logging.error(f"ops_australia_academic_add_save: {e}")
...
def register_routes(app):
    ...
    app.add_url_rule(
        '/operations/australia/academic-details/add',
        endpoint='ops_australia_academic_add_page',
        view_func=ops_australia_academic_add_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/australia/academic-details/add',
        endpoint='ops_australia_academic_add_save',
        view_func=ops_australia_academic_add_save,
        methods=['POST'],
    )
    app.add_url_rule(
