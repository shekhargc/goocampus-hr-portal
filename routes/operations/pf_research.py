"""
routes/operations/pf_research.py — Operations: Portfolio Research & Publication.

Surfaces ops_research_publication rows WHERE pathway='portfolio'. LEFT JOINs
...
@admin_required
def ops_portfolio_research_list():
    """Portfolio research & publication list — ops_research_publication WHERE pathway='portfolio'."""
    user = get_user()
    conn = get_db()
        ...
        statuses = [
            r['research_status'] for r in conn.execute(
                """SELECT DISTINCT research_status FROM ops_research_publication
                    WHERE pathway = 'portfolio' AND research_status IS NOT NULL
                      AND research_status != ''
        ...
        providers = [
            r['research_provider'] for r in conn.execute(
                """SELECT DISTINCT research_provider FROM ops_research_publication
                    WHERE pathway = 'portfolio' AND research_provider IS NOT NULL
        ...
        batches = [
            r['research_batch'] for r in conn.execute(
                """SELECT DISTINCT research_batch FROM ops_research_publication
                    WHERE pathway = 'portfolio' AND research_batch IS NOT NULL
                      AND research_batch != ''
                    ORDER BY research_batch"""
            ).fetchall()
        ]
    except Exception as e:
        logging.error(f"ops_portfolio_research_list: {e}")
        flash(f'Error loading Portfolio research records: {e}', 'error')
    finally:
        conn.close()

    return render_template(
        'ops_portfolio_research_list.html',
        user=user,
        records=records,
        total=total,
        search=search,
        ...
        active_ops_page='portfolio-research',
        active_pathway='portfolio',
    )


@admin_required
def ops_portfolio_research_detail(rid):
    """Read-only Portfolio research record detail.
    ...
    registration_number (pathway='portfolio' scope) so the candidate name
    ...
    try:
        record = conn.execute(
            ...
            LEFT JOIN plab_clients p
                ...
                WHERE r.id = ?
                  AND COALESCE(r.pathway, 'plab') = 'portfolio' """,
            (rid,),
        ).fetchone()
    except Exception as e:
        logging.error(f"ops_portfolio_research_detail: {e}")
    finally:
        conn.close()

    if not record:
        flash('Portfolio research record not found.', 'error')
        return redirect(url_for('ops_portfolio_research_list'))

    return render_template(
        'ops_portfolio_research_detail.html',
        user=user,
        record=record,
        pathway_name='Portfolio Pathway',
        active_ops_page='portfolio-research',
...
@admin_required
def ops_portfolio_research_edit_page(rid):
    """GET — render the edit form for an Portfolio research record."""
    user = get_user()
    conn = get_db()
    record = None
    try:
        ...
        logging.error(f"ops_portfolio_research_edit_page: {e}")
    finally:
        conn.close()
    ...
    return render_template(
        'ops_portfolio_research_edit.html',
        user=user,
        record=record,
        pathway_name='Portfolio Pathway',
        active_ops_page='portfolio-research',
        ...
        **section_research_lookups('portfolio'),
    )


@admin_required
def ops_portfolio_research_edit_save(rid):
    """POST handler: save edits to an Portfolio research record.
    ...
    try:
        ...
        logging.error(f"ops_portfolio_research_edit_save: {e}")
    ...
    return redirect(url_for('ops_portfolio_research_detail', rid=rid))


@admin_required
def ops_portfolio_research_add_page():
...
@admin_required
def ops_portfolio_research_add_save():
    """POST — insert a new Portfolio research / publication row."""
    conn = get_db()
    try:
        reg = (request.form.get('registration_number') or '').strip()
        if not reg:
            flash('Registration number is required.', 'error')
            return redirect(url_for('ops_portfolio_research_add_page'))
        ...
        return redirect(url_for('ops_portfolio_research_detail', rid=new_id))
    except Exception as e:
        logging.error(f"ops_portfolio_research_add_save: {e}")
...
def register_routes(app):
    ...
    app.add_url_rule(
        '/operations/portfolio/research',
        endpoint='ops_portfolio_research_list',
        view_func=ops_portfolio_research_list,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/portfolio/research/<int:rid>',
        endpoint='ops_portfolio_research_detail',
        view_func=ops_portfolio_research_detail,
        methods=['GET'],
    )
    # Add (GET form + POST save)
    app.add_url_rule(
        '/operations/portfolio/research/add',
        endpoint='ops_portfolio_research_add_page',
        view_func=ops_portfolio_research_add_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/portfolio/research/add',
        endpoint='ops_portfolio_research_add_save',
        view_func=ops_portfolio_research_add_save,
        methods=['POST'],
    )
    # Edit page (GET) — renders the form
    app.add_url_rule(
        '/operations/portfolio/research/<int:rid>/edit',
        endpoint='ops_portfolio_research_edit_page',
        view_func=ops_portfolio_research_edit_page,
        methods=['GET'],
    )
    # Save (POST) — same URL, different method, separate endpoint
    app.add_url_rule(
        '/operations/portfolio/research/<int:rid>/edit',
        endpoint='ops_portfolio_research_edit_save',
        view_func=ops_portfolio_research_edit_save,
        methods=['POST'],
    )
