"""
routes/operations/cs_call_notes.py — Operations: Consulting Call Notes list.

Surfaces call-note records from ops_call_notes WHERE pathway='consulting'.
Imported via S-3 data load (see imports/standard_consulting_clients.xlsx)
...
CS_STAGE_FOLLOWUP_DEFAULT = 15


def _cs_stage_interval(stage):
    return CS_STAGE_FOLLOWUP_DAYS.get(stage or '', CS_STAGE_FOLLOWUP_DEFAULT)


def _cs_categorise(days_since, stage):
    """Mirror PLAB's categorise_by_stage. Urgency bands:
       never / critical / overdue / attention / ok / recent."""
    interval = _cs_stage_interval(stage)
...
@admin_required
def ops_consulting_call_notes_list():
    ...
    if (not request.args.get('tab')
        ...
        return redirect(url_for('ops_consulting_call_notes_tracker'))
    ...
    try:
        sql_base = '''SELECT n.id, n.registration_number, n.call_date, n.call_note,
                       ...
                       WHERE COALESCE(n.pathway, 'plab') = 'consulting' '''
        params = []
        if reg:
        ...
        logging.error(f"ops_consulting_call_notes_list: {e}")
    ...
    return render_template(
        'ops_consulting_call_notes_list.html',
        user=user,
        # Notes tab data
        records=records,
        ...
        stage_filter='',
        stage_counts={},
        stage_intervals=CS_STAGE_FOLLOWUP_DAYS,
        # Shared
        pathway_name='Standard Consulting',
        active_tab='notes',
        active_ops_page='consulting-call-notes',
        active_pathway='consulting',
    )


@admin_required
def ops_consulting_call_notes_tracker():
    ...
    try:
        ...
        for row in rows:
            ...
            if not last_call_date or contacted_count == 0:
                ...
                if last_call:
                    ...
                    category = _cs_categorise(days_since, stage)
                else:
                    category = 'never'
        ...
        logging.error(f"ops_consulting_call_notes_tracker: {e}")
    finally:
        try: conn.close()
        except Exception: pass
    ...
    return render_template(
        ...
        stage_counts=stage_counts, stage_intervals=CS_STAGE_FOLLOWUP_DAYS,
...
@admin_required
def ops_consulting_call_notes_not_contacted():
    ...
    try:
             ...
             WHERE COALESCE(p.pathway, 'plab') = 'consulting'
               ...
               AND EXISTS (SELECT 1 FROM ops_call_notes n2
                            WHERE n2.registration_number = p.registration_number
                              AND COALESCE(n2.pathway, 'plab') = 'consulting'
        ...
        logging.error(f"ops_consulting_call_notes_not_contacted: {e}")
    finally:
        try: conn.close()
        except Exception: pass
...
@admin_required
def ops_consulting_call_notes_detail(rid):
    """Full Australia call note view — read-only detail card.

    LEFT JOINs plab_clients via registration_number (pathway='consulting') so
    ...
    if not record:
        ...
        return redirect(url_for('ops_consulting_call_notes_list'))
    return render_template(
        'ops_consulting_call_notes_detail.html',
        user=user,
        record=record,
        pathway_name='Standard Consulting',
        active_ops_page='consulting-call-notes',
...
@admin_required
def ops_consulting_call_notes_edit_page(rid):
    ...
    if not record:
        ...
        return redirect(url_for('ops_consulting_call_notes_list'))
    return render_template(
        'ops_consulting_call_notes_edit.html',
        user=user,
        record=record,
        pathway_name='Standard Consulting',
        active_ops_page='consulting-call-notes',
...
@admin_required
def ops_consulting_call_notes_edit_save(rid):
    """POST handler: save changes to an Australia call note's editable fields.

    Strict allowlist (CS_CALL_NOTES_EDITABLE_COLUMNS). Pathway is NEVER
    ...
    try:
        ...
        sets = []
        params = []
        for col in CS_CALL_NOTES_EDITABLE_COLUMNS:
            if col in request.form:
                val = request.form.get(col, '').strip()
                if col in CS_CALL_NOTES_NUMERIC_COLUMNS:
                    try:
        ...
        logging.error(f"ops_consulting_call_notes_edit_save: {e}")
...
def ops_consulting_call_notes_add():
    ...
    if request.method == 'POST':
        ...
        if not registration_number:
            ...
            return redirect(url_for('ops_consulting_call_notes_add'))
        if not call_date:
        ...
        try:
            client = conn.execute(
                ...
                "WHERE registration_number = ? AND COALESCE(pathway, 'plab') = 'consulting'",
                (registration_number,),
            ).fetchone()
            if not client:
                flash('Selected client is not an Standard Consulting client.', 'error')
                try: conn.rollback()
                except Exception: pass
                return redirect(url_for('ops_consulting_call_notes_add'))
            ...
            logging.error(f"ops_consulting_call_notes_add: {e}")
            try: conn.rollback()
            except Exception: pass
    ...
    return render_template(
        'ops_consulting_call_notes_add.html',
        user=user,
        pre_reg=pre_reg,
...
def register_routes(app):
    ...
    app.add_url_rule(
        '/operations/consulting/call-notes',
        endpoint='ops_consulting_call_notes_list',
        view_func=ops_consulting_call_notes_list,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/call-notes/<int:rid>',
        endpoint='ops_consulting_call_notes_detail',
        view_func=ops_consulting_call_notes_detail,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/call-notes/<int:rid>/edit',
        endpoint='ops_consulting_call_notes_edit_page',
        view_func=ops_consulting_call_notes_edit_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/call-notes/<int:rid>/edit',
        endpoint='ops_consulting_call_notes_edit_save',
        view_func=ops_consulting_call_notes_edit_save,
        methods=['POST'],
    )
    # Add form — mirrors PLAB /operations/call-notes/add structure.
    app.add_url_rule(
        '/operations/consulting/call-notes/add',
        endpoint='ops_consulting_call_notes_add',
        view_func=ops_consulting_call_notes_add,
        methods=['GET', 'POST'],
    )
    ...
    app.add_url_rule(
        '/operations/consulting/call-notes/tracker',
        endpoint='ops_consulting_call_notes_tracker',
        view_func=ops_consulting_call_notes_tracker,
        methods=['GET'],
    )
    # Not Contacted tab — Australia clients with contacted_status='No' notes.
    app.add_url_rule(
        '/operations/consulting/call-notes/not-contacted',
        endpoint='ops_consulting_call_notes_not_contacted',
        view_func=ops_consulting_call_notes_not_contacted,
        methods=['GET'],
    )
