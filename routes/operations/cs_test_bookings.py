"""
routes/operations/cs_test_bookings.py — Operations: Consulting Test Bookings.

Standardizes the Consulting Test Bookings section to match the Consulting
...
All routes are pathway-scoped: ``COALESCE(pathway, 'plab') = 'consulting'``,
so PLAB rows can never leak into the Consulting UI.

Endpoint names (stable so existing url_for() call sites keep working):
  ops_consulting_test_bookings_list         GET /operations/consulting/test-bookings
  ops_consulting_test_bookings_detail       GET /operations/consulting/test-bookings/<rid>
  ops_consulting_test_bookings_edit_page    GET /operations/consulting/test-bookings/<rid>/edit
  ops_consulting_test_bookings_edit_save   POST /operations/consulting/test-bookings/<rid>/edit
"""
...
# Pathway-scoped dropdown options for the Consulting Test Bookings edit form.
from routes.operations._form_lookups import section_test_bookings_lookups


# ── Editable columns on ops_test_bookings (Consulting scope) ────────────────
...
CS_TB_NUMERIC_COLUMNS = {'score', 'reval_score', 'amount', 'points', 'duration'}
...
# ────────────────────────────────────────────────────────────────────────────
@admin_required
def ops_consulting_test_bookings_list():
    """Consulting test bookings list — ops_test_bookings WHERE pathway='consulting'.
    ...
    try:
        ...
        logging.error(f"ops_consulting_test_bookings_list: {e}")
        flash(f'Error loading Consulting test bookings: {e}', 'error')
    finally:
        conn.close()

    return render_template(
        'ops_consulting_test_bookings_list.html',
        user=user,
        records=records,
        total=total,
        search=search,
        ...
        active_ops_page='consulting-test-bookings',
...
# ────────────────────────────────────────────────────────────────────────────
@admin_required
def ops_consulting_test_bookings_detail(rid):
    """Full Consulting test booking profile — read-only PLAB-style layout."""
    user = get_user()
    conn = get_db()
    record = conn.execute(
    ...
    conn.close()
    if not record:
        flash('Consulting test booking not found.', 'error')
        return redirect(url_for('ops_consulting_test_bookings_list'))
    return render_template(
        'ops_consulting_test_bookings_detail.html',
        user=user,
        record=record,
        pathway_name='Standard Consulting',
        active_ops_page='consulting-test-bookings',
...
# ────────────────────────────────────────────────────────────────────────────
@admin_required
def ops_consulting_test_bookings_edit_page(rid):
    """GET — render the edit form for one Consulting test booking."""
    user = get_user()
    conn = get_db()
    record = conn.execute(
        """SELECT t.*, p.prefix, p.first_name, p.last_name
    ...
    if not record:
        ...
        return redirect(url_for('ops_consulting_test_bookings_list'))
    return render_template(
        'ops_consulting_test_bookings_edit.html',
        user=user,
        record=record,
        pathway_name='Standard Consulting',
        active_ops_page='consulting-test-bookings',
        ...
        **section_test_bookings_lookups('consulting'),
    )
...
# ────────────────────────────────────────────────────────────────────────────
@admin_required
def ops_consulting_test_bookings_edit_save(rid):
    """POST handler: save edits to a single Consulting test booking.

    Strict allowlist (CS_TB_EDITABLE_COLUMNS); columns the live schema
    ...
    try:
        ...
        effective = [c for c in CS_TB_EDITABLE_COLUMNS if (not live_cols) or c in live_cols]
        ...
        for col in effective:
            if col in request.form:
                ...
                if col in CS_TB_NUMERIC_COLUMNS:
                    try:
        ...
        logging.error(f"ops_consulting_test_bookings_edit_save: {e}")
    ...
    return redirect(url_for('ops_consulting_test_bookings_detail', rid=rid))
...
# ────────────────────────────────────────────────────────────────────────────
@admin_required
def ops_consulting_test_bookings_add_page():
    """GET — render the empty add form (reuses the edit template)."""
    user = get_user()
    empty_record = {col: None for col in CS_TB_EDITABLE_COLUMNS}
...
# ────────────────────────────────────────────────────────────────────────────
@admin_required
def ops_consulting_test_bookings_add_save():
    """POST — insert a new Consulting test booking row."""
    conn = get_db()
    try:
        reg = (request.form.get('registration_number') or '').strip()
        if not reg:
            flash('Registration number is required.', 'error')
            return redirect(url_for('ops_consulting_test_bookings_add_page'))
        live_cols = _existing_columns(conn, 'ops_test_bookings')
        effective = [c for c in CS_TB_EDITABLE_COLUMNS if (not live_cols) or c in live_cols]
        ...
        return redirect(url_for('ops_consulting_test_bookings_detail', rid=new_id))
    except Exception as e:
        logging.error(f"ops_consulting_test_bookings_add_save: {e}")
...
def register_routes(app):
    ...
    app.add_url_rule(
        '/operations/consulting/test-bookings',
        endpoint='ops_consulting_test_bookings_list',
        view_func=ops_consulting_test_bookings_list,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/test-bookings/<int:rid>',
        endpoint='ops_consulting_test_bookings_detail',
        view_func=ops_consulting_test_bookings_detail,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/test-bookings/add',
        endpoint='ops_consulting_test_bookings_add_page',
        view_func=ops_consulting_test_bookings_add_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/test-bookings/add',
        endpoint='ops_consulting_test_bookings_add_save',
        view_func=ops_consulting_test_bookings_add_save,
        methods=['POST'],
    )
    app.add_url_rule(
        '/operations/consulting/test-bookings/<int:rid>/edit',
        endpoint='ops_consulting_test_bookings_edit_page',
        view_func=ops_consulting_test_bookings_edit_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/consulting/test-bookings/<int:rid>/edit',
        endpoint='ops_consulting_test_bookings_edit_save',
        view_func=ops_consulting_test_bookings_edit_save,
        methods=['POST'],
    )
