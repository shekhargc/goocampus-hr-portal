...
# Pathway-scoped dropdown options for the AMC EPIC edit form.
...
def ops_australia_epic_edit_page(rid):
    ...
    return render_template(
        ...
        **section_epic_lookups('australia'),
    )
...
@admin_required
def ops_australia_epic_add_page():
    """GET — render the empty add form (reuses the edit template)."""
    user = get_user()
    empty_record = {col: None for col in AU_EPIC_EDITABLE_COLUMNS}
...
@admin_required
def ops_australia_epic_add_save():
    """POST — insert a new Australia EPIC registration row."""
    conn = get_db()
    try:
        reg = (request.form.get('registration_number') or '').strip()
        if not reg:
            flash('Registration number is required.', 'error')
            return redirect(url_for('ops_australia_epic_add_page'))
        ...
        logging.error(f"ops_australia_epic_add_save: {e}")
...
def register_routes(app):
    ...
    app.add_url_rule(
        '/operations/australia/epic/add',
        endpoint='ops_australia_epic_add_page',
        view_func=ops_australia_epic_add_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/australia/epic/add',
        endpoint='ops_australia_epic_add_save',
        view_func=ops_australia_epic_add_save,
        methods=['POST'],
    )
    app.add_url_rule(
