"""
routes/operations/pf_payments.py — Portfolio Pathway: Payments list + detail + edit.

Surfaces rows from ops_payments WHERE pathway='portfolio', joined to
plab_clients on registration_number so each row shows the candidate's
human name alongside the payment record.

Endpoint name: ops_portfolio_payments_list. Sidebar pathway switcher
links to /operations/portfolio/payments.

Detail + edit routes mirror the portfolio Registration pattern
(ops_portfolio_clients_list / _edit_page / _edit_save) so the UX is
identical across every portfolio section.
"""
...
# Pathway-scoped dropdown options for the Portfolio Payments edit form
# (Plan Type / Instalment / Payment Method <select>s).
from routes.operations._form_lookups import section_payment_lookups


# ── Editable columns on ops_payments (pathway='portfolio' scope) ──
...
@admin_required
def ops_portfolio_payments_list():
    """Portfolio payments list — ops_payments WHERE pathway='portfolio'."""
    user = get_user()
    conn = get_db()
        ...
        instalments = [
            r['instalment'] for r in conn.execute(
                """SELECT DISTINCT instalment FROM ops_payments
                    WHERE pathway = 'portfolio' AND instalment IS NOT NULL AND instalment != ''
                    ORDER BY instalment"""
            ).fetchall()
        ]
        methods = [
            r['payment_method'] for r in conn.execute(
                """SELECT DISTINCT payment_method FROM ops_payments
                    WHERE pathway = 'portfolio' AND payment_method IS NOT NULL AND payment_method != ''
                    ORDER BY payment_method"""
            ).fetchall()
        ]
    except Exception as e:
        logging.error(f"ops_portfolio_payments_list: {e}")
        flash(f'Error loading Portfolio payments: {e}', 'error')
    finally:
        conn.close()

    return render_template(
        'ops_portfolio_payments_list.html',
        user=user,
        records=records,
        total=total,
        ...
        active_ops_page='portfolio-payments',
        active_pathway='portfolio',
    )


@admin_required
def ops_portfolio_payments_detail(rid):
    """Read-only detail view for a single Portfolio payment row."""
    user = get_user()
    conn = get_db()
    try:
        record = conn.execute(
    ...
    if not record:
        flash('Portfolio payment record not found.', 'error')
        return redirect(url_for('ops_portfolio_payments_list'))

    return render_template(
        'ops_portfolio_payments_detail.html',
        user=user,
        record=record,
        pathway_name='Portfolio Pathway',
        active_ops_page='portfolio-payments',
...
@admin_required
def ops_portfolio_payments_edit_page(rid):
    """GET — render the edit form for an Portfolio payment row."""
    user = get_user()
    conn = get_db()
    try:
        record = conn.execute(
    ...
    return render_template(
        'ops_portfolio_payments_edit.html',
        user=user,
        record=record,
        pathway_name='Portfolio Pathway',
        active_ops_page='portfolio-payments',
        ...
        **section_payment_lookups('portfolio'),
    )


@admin_required
def ops_portfolio_payments_edit_save(rid):
    """POST — save edits to an Portfolio payment row.
    ...
    try:
        ...
        logging.error(f"ops_portfolio_payments_edit_save: {e}")
    ...
    return redirect(url_for('ops_portfolio_payments_detail', rid=rid))


def register_routes(app):
    """Attach this sub-area's URL rules to the Flask app."""
    app.add_url_rule(
        '/operations/portfolio/payments',
        endpoint='ops_portfolio_payments_list',
        view_func=ops_portfolio_payments_list,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/portfolio/payments/<int:rid>',
        endpoint='ops_portfolio_payments_detail',
        view_func=ops_portfolio_payments_detail,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/portfolio/payments/<int:rid>/edit',
        endpoint='ops_portfolio_payments_edit_page',
        view_func=ops_portfolio_payments_edit_page,
        methods=['GET'],
    )
    app.add_url_rule(
        '/operations/portfolio/payments/<int:rid>/edit',
        endpoint='ops_portfolio_payments_edit_save',
        view_func=ops_portfolio_payments_edit_save,
        methods=['POST'],
    )
